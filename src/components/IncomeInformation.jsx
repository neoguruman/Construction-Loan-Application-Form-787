...
const IncomeInformation = () => {
  ...
  const calculateHousingTotal = (type) => {
    if (type === 'present') {
      return parseFloat((formData.currentRent || '').replace(/[$,]/g, '')) || 0;
    } else {
      const expenses = [
        formData.proposedMortgage,
  ...
  const renderExpenseField = (field, isPresent = false) => {
    return (
      <div className="relative">
        {!isPresent ? (
          <div className="px-3 py-2 bg-gray-100 rounded text-gray-500">N/A</div>
        ) : (
          <div className="flex items-center">
            <input
              type="text"
  ...
  return (
    ...
    >
      <h2 className="text-3xl font-bold text-gray-800 mb-8">Monthly Income and Combined Housing Expense Information</h2>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gray-50 p-6 rounded-lg">
          <h3 className="text-xl font-semibold text-gray-800 mb-6">Combined Monthly Housing Expense</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="font-medium text-gray-700">Expense Type</div>
            <div className="font-medium text-gray-700">Present</div>
            <div className="font-medium text-gray-700">Proposed</div>
            ...
            <div className="text-sm text-gray-600">First Mortgage (P&I)</div>
            <div className="px-3 py-2 bg-gray-100 rounded text-gray-500">N/A</div>
            <div className="flex items-center">
              <input
                type="text"
                value={formData.proposedMortgage || ''}
                onChange={(e) => updateFormData('proposedMortgage', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="$0.00"
              />
              <button 
                onClick={() => clearFormField('proposedMortgage')}
                className="ml-1 bg-red-500 hover:bg-red-600 text-white p-2 rounded-lg"
                title="Clear field"
              >
                <SafeIcon icon={FiX} className="w-3 h-3" />
            ...
            {[
              { label: 'Other Financing (P&I)', field: 'otherFinancing' },
              { label: 'Hazard Insurance', field: 'hazardInsurance' },
              { label: 'Real Estate Taxes', field: 'realEstateTaxes' },
              { label: 'Mortgage Insurance', field: 'mortgageInsurance' },
              { label: 'Homeowner Assn. Dues', field: 'hoaDues' },
              { label: 'Other', field: 'otherExpenses' }
            ].map(item => (
            ...
            <div className="font-bold text-gray-800 border-t pt-2">Total</div>
            <div className="font-bold text-blue-600 border-t pt-2 bg-blue-50 px-3 py-2 rounded">
              {formatCurrency(calculateHousingTotal('present'))}
            </div>
            <div className="font-bold text-blue-600 border-t pt-2 bg-blue-50 px-3 py-2 rounded">
              {formatCurrency(calculateHousingTotal('proposed'))}
            </div>
          </div>
        </div>
      </div>
